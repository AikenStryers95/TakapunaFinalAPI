$(function() {
    ...
    let clientid = '&client_id=R0W2TWQHMLJDLVGUIKQGS2CHPWSG2IZZHBTFLS0UZ1DRG5CY';
    let clientSecret = '&client_secret=YGRHHTLVC1WVNL32HOX4T1U5IGNHBI3X2XW0RMWG5SSL2UX0';
    ...
    function searchVenues(type){

        let exploreUrl = 'https://api.foursquare.com/v2/venues/explore'+key+'&sortByDistance=1&near=-36.78687%2C174.777133&query='+type;
        ...
        $.ajax({
            ...
            success:function(res){
                ...
                _(venues).each(function(venue){
                    ...
                    if(result.length > 0){
                        let marker = L.marker(venue.latlng,{icon:venueIcon}).addTo(group);
                        ...
                        marker.on('click',function(){
                            ...
                            $.ajax({
                                ...
                                success:function(res){
                                    ...
                                    let link = res.response.venue.canonicalUrl;
                                    let photos = [];
                                    if(res.response.venue.photos.groups[0]){
                                       photos = res.response.venue.photos.groups[0].items; 
                                    }
                               
                                    let address = res.response.venue.location.formattedAddress;
                                    let rating = res.response.venue.rating;
                                    let likes = res.response.venue.likes.count;
                                    let contact = res.response.venue.contact.formattedPhone;


                                    $('#myModal .modal-title a').text(name).attr('href',link);

                                    _(photos).each(function(photo){
                                        $('<img src="'+photo.prefix+'150x150'+photo.suffix+'">').appendTo('.modal-body');
                                    });

                                    $('#myModal .modal-footer1').text('Rating: '+rating+' /10');
                                    $('#myModal .modal-footer').text('Address:  ');
                                    $('<a href="https://www.google.com/maps/dir/'+currentLocation+'/'+venue.latlng+'" target=blank>' + address+'</a>').appendTo('.modal-footer');
                                    $('#myModal .modal-footer2').text('Number:  ');
                                     $('<a href="#">' + contact+'</a>').appendTo('.modal-footer2');
